...
/// BSC transaction info
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BscTxInfo {
    /// Transaction hash
    pub tx_hash: H256,
...
impl BscConnector {
    ...
    pub async fn new(rpc_url: &str, network_name: String, chain_id: u64) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        info!("BSC connector initialized for network: {}", network_name);
    ...
    pub async fn update_gas_price(&self) -> Result<(), Box<dyn std::error::Error>> {
        // For BSC, we use a different approach to get gas price
    ...
    pub async fn get_transaction_info(&self, tx_hash: H256) -> Result<Option<BscTxInfo>, Box<dyn std::error::Error>> {
    ...
    pub async fn send_transaction(
        ...
        info!("Sent BSC transaction: {:?}", tx_hash);
...
/// Convert public key to BSC address
