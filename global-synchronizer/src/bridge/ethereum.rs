...
impl EthereumConnector {
    ...
    pub async fn new(rpc_url: &str, network_name: String, chain_id: u64) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        info!("Ethereum connector initialized for network: {}", network_name);
    ...
    pub async fn get_transaction_info(&self, tx_hash: H256) -> Result<Option<EthereumTxInfo>, Box<dyn std::error::Error>> {
        let tx = self.client.eth().transaction(web3::types::TransactionId::Hash(tx_hash)).await?;
        
        if let Some(tx) = tx {
            let receipt = self.get_transaction_receipt(tx_hash).await?;
            
            let info = EthereumTxInfo {
                tx_hash: tx.hash,
                from: tx.from,
                to: tx.to,
                value: tx.value,
    ...
    pub async fn send_transaction(
        ...
        info!("Sent Ethereum transaction: {:?}", tx_hash);
